// Question pool for What the Quack.
//
// Structure: CATEGORIES[categoryKey].stages[stageKey] = Question[]
// Question = { text: string, tags: string[] }
//
// `tags` are the words/phrases this question is "about" — they're run
// through the same tokenizer/stemmer as the user's notes, so a note
// mentioning "cache" or "cached" will both match a question tagged
// "cache". Tags exist purely for relevance scoring (see
// src/nlp/selectQuestion.js) — they're not shown in the UI.
//
// The category is a one-time choice the user makes up front (what kind
// of bug is this?). It exists because "explain the problem" questions
// that work for a regression don't work for a heisenbug, and vice
// versa. Within a category, the four stages walk from understanding
// the problem toward verifying a fix, mirroring how experienced
// developers actually debug rather than a generic checklist.
//
// To add a question: pick the right category + stage array and add an
// object with `text` and a handful of `tags`. Keep it a genuine open
// question, not a leading one — it should make the reader think, not
// hint at the answer.

export const STAGE_ORDER = ["understand", "isolate", "test", "verify"];
...
export const CATEGORIES = {
  ...
  neverWorked: {
    ...
    hint: "new code, and it's not doing what you expected",
    stages: {
      understand: [
        ...
        { text: "Is there a version of this same feature elsewhere — another language, library, or past project — that behaves the way you expected?", tags: ["reference", "prior experience", "comparison"] },
      ...
      isolate: [
        ...
        { text: "Does the problem exist at the boundary — inputs and outputs — or inside the logic itself?", tags: ["boundary", "input", "logic"] },
      ],
      test: [
  ...
  intermittent: {
    ...
    stages: {
      ...
      isolate: [
        ...
        { text: "Does the order you run things in — tests, requests, initialization — ever change between passes and fails?", tags: ["order", "initialization", "sequence"] },
        { text: "Is there any resource — memory, connection pool, file handle — that could be exhausted only some of the time?", tags: ["resource", "exhaustion", "pool"] },
      ...
      test: [
        ...
        { text: "If you pin down timing artificially — a mocked clock, a forced delay — can you make it fail on command?", tags: ["mocked clock", "forced delay", "reproduce"] },
        ...
        { text: "Have you correlated failures against a timestamp or log from an external system — queue, database, network?", tags: ["correlate", "external system", "logs"] },
      ],
      verify: [
  ...
  silentWrong: {
    ...
    hint: "it runs fine, but the output or behavior is incorrect",
    stages: {
      understand: [
        ...
        { text: "At what point does 'correct' data become 'wrong' — where's the first sign of it?", tags: ["data", "correct", "trace", "location"] },
        ...
        { text: "Does the wrongness show up immediately, or only after some accumulation — rounding, aggregation, repeated operations?", tags: ["accumulation", "rounding", "aggregation"] },
      ...
      test: [
        ...
        { text: "Are you comparing the right things — same units, same rounding, same timezone?", tags: ["units", "rounding", "timezone", "comparison"] },
